...
void GenerateEller (std::vector<std::vector<char>>& map, unsigned width, unsigned height)
{
	...
	for (unsigned i = 0; i < height; ++i)
	{
		...
		for (unsigned j = 0; j < width - 1; ++j)
		{
			...
			if ((right_wall == 1) || (row_set[j] == row_set[j + 1]))
				map[i * 2 + 1/*верхний ряд в i-ом ряду ячеек 2x2*/][j * 2 + 2/*Правый столбец в (i;j) ячейке 2x2*/] = '#';/*Создаем стену*/
			else
			{
		...
		if (i != height - 1)
		{
			...
			for (unsigned j = 0; j < width; ++j) {
				...
				for (unsigned l = 0; l < width; ++l)
					if ((row_set[l] == row_set[j]) && (map[i * 2 + 2][l * 2 + 1] == ' '))
						count_hole++;
				if (count_hole == 0)
					map[i * 2 + 2][j * 2 + 1] = ' ';
			}
			...
			for (unsigned j = 0; j < width; ++j)
				if (map[i * 2 + 2/* Проверим надичие нижней стены у текущего ряда*/][j * 2 + 1] == '#')
